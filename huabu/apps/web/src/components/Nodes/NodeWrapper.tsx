...
import {
  ...
  type LcosNodePresentationInput,
} from '@/lcos-seam/nodePresentation';
...
export const NodeWrapper = memo(
  ({
    ...
    // 是否正在被本节点的手柄拖拽缩放——只服务于呈现层 phase 判定（LCOS 自适应密度）。
    ...
    // LCOS 薄接缝（Wave 9）：向 host 渲染器发布中性呈现输入（world 尺寸 × zoom、
    // DPR、交互 phase）。这是 web-gen2 自适应密度解析器唯一的真实数据来源——
    // 密度按屏幕像素判定，不由本组件或 host 侧各自的 zoom 阶梯另立一套。
    // 不在此读取任何 LCOS 领域语义；未挂 host 扩展时无消费者，纯惰性。
    const lcosPresentation = useMemo<LcosNodePresentationInput>(
      () => ({
    ...
    // Check if this node was generated by AI. LCOS owns provenance through its
    // species body, so the legacy Huabu badge must not leak through fallbacks.
    const isAIGenerated = data.origin?.type?.startsWith('ai-');
    const showAIBadge =
      chromeMode !== 'lcos' &&
      (hostPresentation?.showAiBadge ?? true) &&
      isAIGenerated &&
      nodeWidth * zoom >= AI_BADGE_MIN_SCREEN_WIDTH;
    ...
    const standDownLegacyToolbar = shouldStandDownLegacyNodeToolbar(chromeMode, type, hostPresentation !== undefined);
      ...
      <>
        ...
        <div
          ...
          className={cn(
            ...
            (!nativeSurfaceEnabled || (!accentTokens && !fillColor)) && 'bg-transparent',
          ...
          style={{
            // A host body may use percentage height while the native carrier
            // (notably text) has no materialized layout height. Give that
            // body the same measured/default height used by presentation;
            // do not write geometry or replace an explicit user height.
            ...(hostPresentation && !hasLayoutHeight ? { height: nodeHeight } : {}),
            ...
            ...(nativeSurfaceEnabled && accentTokens ? { borderColor: accentTokens.border } : {}),
            ...(type === 'question' && {
          ...
          */}
          ...
          <div
            className={clsx(
              ...
              hasLayoutHeight || hostPresentation ? 'min-h-0 flex-1' : 'min-h-0',

              effectiveAllowOverflow ? 'overflow-visible' : 'overflow-hidden rounded-md',
            )}
          >
            {/*
              LCOS host 渲染器（body 替换 / 物种形态）挂在这里，因此呈现输入
              必须由本层发布，消费者才能读到活的 world×zoom 与 phase。
              Huabu 原生 body 不受影响（provider 只是透传，无消费者即无成本）。
            */}
            <LcosNodePresentationProvider value={lcosPresentation}>
              {children}
            </LcosNodePresentationProvider>
          </div>
