    ...
    <div data-lcos-reader className="flex h-full flex-col gap-3 p-5" style={{ minHeight: 320 }}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="truncate text-base font-semibold" style={{ color: lcosTokens.color.text.light }}>
            {detail.artifact.title ?? '未命名'}
          </h3>
          <p className="mt-0.5 break-all text-xs" style={{ color: lcosTokens.color.muted.light }}>
      ...
      {detail.revisions.length > 1 && (
        <div className="flex flex-wrap gap-1">
          {detail.revisions.map((r) => (
            <span key={r.id} className="rounded-full px-2 py-0.5 text-[10px]" style={{ background: lcosTokens.color.raised.light, color: lcosTokens.color.muted.light }}>
              {r.id.slice(0, 8)}
            </span>
          ))}
        </div>
  ...
  return (
    <div className="flex h-full min-h-[240px] items-center justify-center">
      {feedback === 'loading' || feedback === 'error' ? (
        ...
        <span className="text-sm" style={{ color: lcosTokens.color.muted.light }}>{text}</span>
      )}
    </div>
  );
}