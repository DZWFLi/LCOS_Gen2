...
|---|---|---|---|---|---|---|---|---|---|
| T1-A01 | Canvas kernel boundary | Huabu `components/Panels/Canvas/Canvas.tsx::Canvas` + ReactFlow | C | `lcos/host/CanvasHostBoundary.tsx` | `LcosProjectShell → LcosWorksiteStage → CanvasHostBoundary` | LCOS mode 下旧 chrome 不挂 | Huabu kernel | PLANNED | 待 Wave 2 |
| T1-A02 | Node body seam | Huabu `NodeWrapper.tsx::NodeWrapper` + `lcos-seam/nodeBodySlot.tsx` | B | `lcos/nodes/LcosNodePresentationProvider.tsx` | 各 native node body junction | native body 降 fallback | Huabu geometry | PLANNED | 待 Wave 3 |
